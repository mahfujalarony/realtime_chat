import { Download, FileText, Loader2, Mic, Paperclip, Pause, Play, Square, Trash2 } from 'lucide-react'
...
function AudioMessageBubble({ url, fallbackDurationSec }) {
  ...
  const [duration, setDuration] = useState(
    Number.isFinite(Number(fallbackDurationSec)) ? Number(fallbackDurationSec) : 0,
  )
    ...
    <section className={`w-full min-h-0 flex-col ${isMobileChatOpen ? 'flex' : 'hidden md:flex md:flex-col'}`}>
      <header className="flex items-center gap-3 border-b border-[#e4e4e4] bg-[#f0f2f5] px-4 py-3">
        {/* <button
          type="button"
          onClick={backToList}
          className="rounded-md px-2 py-1 text-sm font-medium text-[#54656f] md:hidden"
        >
          {'< Back'}
        </button> */}
        ...
        {activeChat ? (
          <>
            <button
              type="button"
              onClick={openProfile}
              className="flex h-10 w-10 items-center justify-center rounded-full bg-[#d0d7db] text-sm font-semibold text-[#30424f]"
            >
              {getInitials(activeChat.username)}
            </button>
            <button type="button" onClick={openProfile} className="text-left">
              <p className="text-sm font-semibold text-[#1f2c34]">{activeChat.username}</p>
              <p className="text-xs text-[#667781]">
                {activeChat.lastSeen ? `last seen ${new Date(activeChat.lastSeen).toLocaleString()}` : 'offline'}
              </p>
            </button>
          ...
          <p className="text-sm text-[#667781]">Select a user to start chatting</p>
        )}
      </header>
                  ...
                  >
                    {message.mediaUrl && message.messageType === 'image' ? (
                      <img
                        ...
                        className="mb-1 max-h-64 w-auto cursor-pointer rounded-md object-cover"
                        onClick={() => window.open(message.mediaUrl, '_blank')}
                      />
                    ) : null}

                    {message.mediaUrl && message.messageType === 'video' ? (
                      <video src={message.mediaUrl} controls className="mb-1 max-h-72 w-full rounded-md" />
                    ) : null}
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-medium text-[#111b21]">
                            {getFileName(message.mediaUrl, message.mediaOriginalName)}
                          </p>
                          <p className="text-xs text-[#667781]">
                    ...
                    {message.text ? (
                      <p className="break-words whitespace-pre-wrap text-[14.2px] leading-[19px] text-[#111b21]">
                        {message.text}
                      </p>
                    ) : null}
                    <div className="mt-1 -mb-1 flex items-center justify-end gap-1">
                      ...
                      {isMine && (
                        <svg viewBox="0 0 16 11" height="11" width="16" className="text-[#53bdeb]">
            ...
            <button
              ...
              className={`rounded-lg p-2 transition ${
                isRecording
                  ...
                  : 'bg-white text-[#54656f] hover:bg-[#edf0f2]'
